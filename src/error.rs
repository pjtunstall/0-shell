...
// POSIX guarantees write(2) is async-signal-safe, so calling `libc::write` in a signal handler or post-fork child is allowed. `println!` goes through Rust’s buffered stdout, which takes locks and can allocate, so it's not async-signal-safe.
